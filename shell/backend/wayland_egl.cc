...
void WaylandEglBackend::EGL_KHR_debug_init() {
...
void* WaylandEglBackend::get_egl_proc_address(const char* address) {
  const char* extensions = eglQueryString(EGL_NO_DISPLAY, EGL_EXTENSIONS);
...
bool WaylandEglBackend::MakeCurrent(size_t index) {
...
bool WaylandEglBackend::ClearCurrent() {
...
bool WaylandEglBackend::SwapBuffers(size_t index) {
...
bool WaylandEglBackend::MakeResourceCurrent(size_t index) {
...
bool WaylandEglBackend::MakeTextureCurrent() {
...
void* WaylandEglBackend::gl_process_resolver(const char* name) {
  if (name == nullptr) {
